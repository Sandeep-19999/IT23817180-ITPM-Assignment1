...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        # 4. Iterate Rows
        processed = 0
        row_start = int(args.start_row or 0)
        row_end = int(args.end_row or 0)

        if row_start <= 0:
            row_start = header_row + 1
        if row_end <= 0:
            row_end = int(ws.max_row or 0)

        row_start = max(header_row + 1, row_start)
        row_end = min(int(ws.max_row or 0), row_end)
